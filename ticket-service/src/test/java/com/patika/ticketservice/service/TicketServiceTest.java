...
class TicketServiceTest {
    ...
    void createTicket_ShouldCreateTicket_WhenValidRequest() {
        ...
        assertThat(response.getPassengerFirstName()).isEqualTo("Bedirhan");
    ...
    void getAllByEmail_ShouldReturnTickets_WhenUserHasBookings() {
        ...
        Mockito.when(userClientService.getUserByEmail("bedirhanbalci@outlook.com")).thenReturn(userResponse);
        Mockito.when(bookingRepository.findByUserId(1L)).thenReturn(Collections.singletonList(booking));

        // when
        List<TicketResponse> responses = ticketService.getAllByEmail("bedirhanbalci@outlook.com");

        // then
        verify(userClientService, times(1)).getUserByEmail("bedirhanbalci@outlook.com");
        verify(bookingRepository, times(1)).findByUserId(1L);
        assertThat(responses).hasSize(1);
        assertThat(responses.get(0).getPassengerFirstName()).isEqualTo("Bedirhan");
    }
}
